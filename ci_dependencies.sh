...
if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list liblua5.1-0-dev >/dev/null 2>&1) || \
       ...
       ([ -e "lua-5.1" ]) \
; then
 echo "INFO: Get 'lua-5.1' using apt-get liblua5.1-0-dev" >&2
 sudo apt-get install liblua5.1-0-dev -y || exit $?
fi
...
if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libsnmp-dev >/dev/null 2>&1) || \
       ...
       ([ -e "netsnmp" ]) \
; then
 echo "INFO: Get 'netsnmp' using apt-get libsnmp-dev" >&2
 sudo apt-get install libsnmp-dev -y || exit $?
fi
